...
impl<'a, F: Function> Env<'a, F> {
    ...
    pub fn merge_bundles(&mut self, from: LiveBundleIndex, to: LiveBundleIndex) -> bool {
        ...
        // Both bundles must agree on a fixed spill slot (or at most one
        // of them names one). Refuse a merge that would put two
        // *different* embedder-fixed homes in the same spillset — each
        // keeps its own slot, and the block-edge move stays explicit.
        let from_fixed = self.spillsets[self.bundles[from].spillset].fixed_slot;
        let to_fixed = self.spillsets[self.bundles[to].spillset].fixed_slot;
        if let (Some(a), Some(b)) = (from_fixed, to_fixed) {
            if a != b {
                trace!(" -> conflicting fixed spill slots ({:?} vs {:?})", a, b);
                return false;
            }
        }
        ...
        if ranges_to.is_empty() {
            ...
            // Propagate the fixed-slot annotation onto the
            // surviving spillset (compatibility was checked at the
            // top; `from`'s spillset is orphaned).
            if from_fixed.is_some() {
                let to_ss = self.ctx.bundles[to].spillset;
                if self.ctx.spillsets[to_ss].fixed_slot.is_none() {
                    self.ctx.spillsets[to_ss].fixed_slot = from_fixed;
                }
            }
            return true;
        }
        ...
        if self.bundles[from].spillset != self.bundles[to].spillset {
            ...
            let to_ss = self.ctx.bundles[to].spillset;
            let to_range = &mut self.ctx.spillsets[to_ss].range;
            *to_range = to_range.join(from_range);
            // Propagate the fixed-slot annotation onto the
            // surviving spillset (compatibility was checked at the
            // top; `from`'s spillset is orphaned).
            if from_fixed.is_some() && self.ctx.spillsets[to_ss].fixed_slot.is_none() {
                self.ctx.spillsets[to_ss].fixed_slot = from_fixed;
            }
        }
    ...
    pub fn merge_vreg_bundles(&mut self) {
        ...
        for vreg in 0..self.vregs.len() {
            ...
            let fixed_slot = self.func.vreg_fixed_spillslot(reg);
