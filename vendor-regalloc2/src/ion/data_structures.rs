...
pub struct SpillSet {
    ...
    /// Embedder-fixed spill slot for every VReg in this set (see
    /// [`crate::Function::vreg_fixed_spillslot`]). Seeded per initial
    /// bundle from its VReg's fixed slot; propagated on merge (a merge
    /// between spillsets with *different* fixed slots is refused so
    /// each keeps its own home). `None` = allocator-assigned slot.
    pub fixed_slot: Option<crate::SpillSlot>,
...
pub struct Ctx {
    ...
    /// One `SpillSlotData` per distinct embedder-fixed slot, so
    /// multiple spillsets that share a fixed home reuse the same
    /// range-btree and `spillslot_can_fit_spillset` catches an
    /// overlapping occupant (correctness: two live values must
    /// not alias one `[fp − r×8]` cell).
    pub(crate) fixed_spillslot_map:
        FxHashMap<crate::SpillSlot, SpillSlotIndex>,
