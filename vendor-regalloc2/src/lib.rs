...
#[cfg(any(feature = "fuzzing", feature = "std"))]
extern crate std;
...
impl SpillSlot {
    ...
    /// Threshold above which a `SpillSlot` index denotes a *fixed*
    /// (embedder-managed) slot rather than an allocator-assigned one.
    /// Auto-allocated slots use indices `0..num_spillslots`; fixed slots
    /// carry an embedder-defined index in `FIXED_BASE..MAX`. The allocator
    /// never assigns a fixed index — it only propagates one returned from
    /// [`Function::vreg_fixed_spillslot`]. The embedder is responsible for
    /// mapping a fixed slot's `fixed_index()` to an actual frame address.
    ///
    /// `1 << 20` gives ~1 M auto slots and ~15 M fixed indices, both well
    /// beyond any function this allocator handles.
    pub const FIXED_BASE: usize = 1 << 20;
    ...
    /// Create a new *fixed* spill slot (see [`FIXED_BASE`]). `idx` is an
    /// embedder-defined index (e.g. a frame-head slot number); the
    /// allocator never interprets it, only carries it through to `Edit`s
    /// and `Allocation`s so the embedder's `gen_spill`/`gen_reload` can
    /// address the slot directly.
    #[inline(always)]
    pub fn new_fixed(idx: usize) -> Self {
        debug_assert!(idx < Self::MAX - Self::FIXED_BASE);
        SpillSlot { bits: (Self::FIXED_BASE + idx) as u32 }
    }

    /// Is this a fixed (embedder-managed) slot?
    #[inline(always)]
    pub fn is_fixed(self) -> bool {
        self.is_valid() && (self.bits as usize) >= Self::FIXED_BASE
    }

    /// The embedder-defined index for a fixed slot (undefined for auto
    /// slots — check `is_fixed()` first).
    #[inline(always)]
    pub fn fixed_index(self) -> usize {
        debug_assert!(self.is_fixed());
        (self.bits as usize) - Self::FIXED_BASE
    }
...
pub trait Function {
    ...
    /// If this VReg's canonical storage location is a *fixed* frame slot
    /// managed by the embedder (rather than an allocator-assigned spill
    /// slot), return it. When set, the allocator will use exactly this
    /// slot as the spill location for every live-range of `vreg` — and,
    /// after move-coalescing, for every VReg merged into the same
    /// spillset. Two VRegs that name *different* fixed slots are never
    /// merged (the merge is refused, so each keeps its own slot).
    ///
    /// The returned slot must be a `SpillSlot::new_fixed(idx)`; auto-
    /// index collisions with allocator-assigned slots are the embedder's
    /// responsibility to avoid (use `new_fixed`, which offsets by
    /// `FIXED_BASE`).
    ///
    /// This is the mechanism for "the JS interpreter frame slot IS the
    /// spill slot": when a VReg models a bytecode local whose home is
    /// `[fp − r×8]`, returning `new_fixed(r)` here makes the allocator
    /// spill to that home instead of a redundant `[sp + N]` slot.
    fn vreg_fixed_spillslot(&self, _vreg: VReg) -> Option<SpillSlot> {
        None
    }
