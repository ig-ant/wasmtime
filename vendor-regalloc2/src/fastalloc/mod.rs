...
impl<'a, F: Function> State<'a, F> {
    ...
    fn get_spillslot(&mut self, vreg: VReg) -> SpillSlot {
        if self.vreg_spillslots[vreg.vreg()].is_invalid() {
            // NOTE: `Function::vreg_fixed_spillslot` is IGNORED
            // here. The single-pass allocator has no range-btree
            // to gate two live vregs sharing one fixed home (ion's
            // `spillslot_can_fit_spillset` fallback), and no
            // poison-sentinel filter — honouring a fixed slot
            // could alias two live values or write `[fp+0]` (the
            // saved fp) for a slot-0 poison. Falling back to auto
            // is always sound; embedders wanting fixed-slot spills
            // must use the backtracking allocator.
